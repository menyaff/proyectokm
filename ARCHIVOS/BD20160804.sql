...
INSERT INTO `cobros` VALUES (1,'2016-07-31 21:58:54',1,200.00,'Test1',1),(2,'2016-07-31 22:00:34',1,200.00,'Test2',1),(3,'2016-07-29 05:00:00',1,150.00,'Test3',1);
...
INSERT INTO `gastos` VALUES (6,1,1,500.00,'Test1',1,NULL,NULL,'2016-07-31 21:25:03',NULL,NULL,NULL,1,'2016-08-02 04:00:27'),(7,1,1,100.00,'Test2',1,NULL,NULL,'2016-07-31 21:26:57',NULL,NULL,NULL,0,NULL),(8,1,1,200.00,'Test3',1,NULL,NULL,'2016-07-31 21:30:18',NULL,NULL,NULL,1,'2016-07-31 21:30:18'),(9,1,1,1000.00,'Test4',1,NULL,NULL,'2016-07-31 21:32:22',NULL,NULL,NULL,1,'2016-07-31 21:52:12'),(10,1,1,100.00,'Test1',2,NULL,NULL,'2016-07-31 21:59:24',NULL,NULL,NULL,1,'2016-07-31 21:59:24');
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` TRIGGER `kmasociados`.`gastos_BEFORE_INSERT` BEFORE INSERT ON `gastos` FOR EACH ROW
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` TRIGGER `kmasociados`.`gastos_BEFORE_UPDATE` BEFORE UPDATE ON `gastos` FOR EACH ROW
BEGIN
	IF(NEW.realizado=1 OR NEW.realizado=TRUE) THEN
		SET NEW.fechaRealizado = CURRENT_TIMESTAMP; 
...
INSERT INTO `rolesmodulos` VALUES (1,1,1),(2,1,2),(3,1,3),(4,1,4),(5,2,1),(6,2,2),(7,2,3),(8,2,4),(9,1,5),(10,1,6),(15,1,9),(16,1,10),(19,1,13),(20,1,14),(21,1,15),(22,1,16),(23,1,17),(24,1,18),(25,1,19),(26,1,20),(27,1,21),(28,1,22),(29,1,23),(30,1,24),(31,1,25),(32,1,26),(34,1,28),(35,2,28),(36,1,29),(37,1,30),(38,1,31),(39,1,32);
...
INSERT INTO `usuarios` VALUES (1,'admin','da39a3ee5e6b4b0d3255bfef95601890afd80709','877ac7ac4615d41d1f72260f477a6f3160db87c6',0,0),(6,'Erika','da39a3ee5e6b4b0d3255bfef95601890afd80709','83abcb60b6209a46d690a8834c3a06cd19039bfe',2,2),(8,'MenyAFF','da39a3ee5e6b4b0d3255bfef95601890afd80709','e29d87fc01e107836b08439da01eef83dfdc61f6',1,1);
...
BEGIN
	SET @sqlQuery = CONCAT("SELECT * FROM(
							SELECT cb.nombre, b.nombre AS banco, cb.noCuenta, cb.clabe, cb.saldo,
							 g.fechaRealizado AS fecha, g.monto, g.concepto,
                            'GASTO' AS movimiento
							FROM cuentasBancarias cb
							LEFT JOIN cat_bancos b ON cb.banco=b.id
							LEFT JOIN gastos g ON g.cuentaBancaria = cb.id AND g.realizado=1 ");	
	IF(P_fechaIni IS NOT NULL AND P_fechaIni!="")THEN
		SET @sqlQuery = CONCAT(@sqlQuery,"AND DATE(g.fechaRealizado)>='",P_fechaIni,"' ");
	END IF;
	IF(P_fechaFin IS NOT NULL AND P_fechaFin!="")THEN
		SET @sqlQuery = CONCAT(@sqlQuery,"AND DATE(g.fechaRealizado)<='",P_fechaFin,"' ");
	END IF;
	SET @sqlQuery = CONCAT(@sqlQuery,"WHERE cb.id=",P_id,"
                            UNION
                            SELECT cb.nombre, b.nombre AS banco, cb.noCuenta, cb.clabe, cb.saldo,
							c.fecha,c.monto, c.concepto,
                            'COBRO' AS movimiento
							FROM cuentasBancarias cb
							LEFT JOIN cat_bancos b ON cb.banco=b.id
							LEFT JOIN cobros c ON c.cuentaBancaria = cb.id ");	
	IF(P_fechaIni IS NOT NULL AND P_fechaIni!="")THEN
		SET @sqlQuery = CONCAT(@sqlQuery,"AND DATE(c.fecha)>='",P_fechaIni,"' ");
	END IF;
	IF(P_fechaFin IS NOT NULL AND P_fechaFin!="")THEN
		SET @sqlQuery = CONCAT(@sqlQuery,"AND DATE(c.fecha)<='",P_fechaFin,"' ");
	END IF;
	SET @sqlQuery = CONCAT(@sqlQuery,"WHERE cb.id=",P_id,") tmp ORDER BY fecha;");
...
-- Dump completed on 2016-08-04  9:51:32
