...
INSERT INTO `usuarios` VALUES (1,'admin','24bd16f242e360fe05f5927c19efc73e63b5a685','684a9ec0c2d467ace5d6ae146953b195524b49dc',0,0),(3,'meny','123456','1a2b3c4d',0,0),(4,'Menyff','7787ee0aa1ba7eff4248b32a1e9f431da09ec7b1','1a2b3c4d',0,54);
...
BEGIN
    ...
    IF(@dependencias=0) THEN         
		...
		SELECT 'TRUE' AS respuesta, NULL AS 'error';
	ELSE
		SELECT 'FALSE' AS respuesta, 'Banco asignado a una cuenta bancaria' AS 'error';
	END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
	...
	SET @sqlQuery = CONCAT("SELECT SUM(info) INTO @dependencias FROM (
								SELECT COUNT(*) AS info FROM eventos WHERE cuentaBancaria IN(",P_id,")
								UNION
								SELECT COUNT(*) AS info FROM gastos WHERE cuentaBancaria IN(",P_id,")
								UNION
								SELECT COUNT(*) AS info FROM cobros WHERE cuentaBancaria IN(",P_id,")
							) tmp;");
    ...
    IF(@dependencias=0) THEN         
		...
		SELECT 'TRUE' AS respuesta, NULL AS 'error';
	ELSE
		SELECT 'FALSE' AS respuesta, 'Cuenta bancaria asignada a un evento, gastos o cobros' AS 'error';
	END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
	...
	SET @sqlQuery = CONCAT("SELECT SUM(info) INTO @dependencias FROM(
									SELECT COUNT(*) AS info FROM gastos WHERE evento IN(",P_id,")
                                    UNION
                                    SELECT COUNT(*) AS info FROM cobros WHERE evento IN(",P_id,")
							) tmp");
    ...
    IF(@dependencias=0) THEN                       
		...
		SELECT 'TRUE' AS respuesta, NULL AS 'error';
	ELSE
		SELECT 'FALSE' AS respuesta, 'Evento asignado a un gasto o un cobro' AS 'error';
	END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
	SET @sqlQuery = "SELECT a.id, a.clave, a.nombre, a.linea, a.marca, a.modelo, a.descripcion, cl.id AS id_lugar, cl.nombre AS lugar, a.codigoBarras, 
							a.sustituto, a.unidad, a.existencias, ca.id AS id_area, ca.nombre AS area, a.archivo, a.precioRecuperacion, a.precioLista, a.precioMayoreo
					FROM articulos a
...
BEGIN
	SET @sqlQuery = "SELECT ae.id, a.id AS id_articulo, a.nombe AS articulo, e.id AS id_evento, e.nombre AS evento, ae.cantidad
...
BEGIN
	SET @sqlQuery = "SELECT e.id, e.nombre, e.cotizacion, csc.id AS id_catStatusCotizacion, csc.nombre AS statusCotizacion, 
					...
					JOIN cat_bancos cb ON e.banco=cb.id";
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SPU_usuarios`(IN P_id INT, IN P_accion VARCHAR(50), IN P_nombre VARCHAR(50), IN P_pass VARCHAR(100), IN P_llave VARCHAR(100), IN P_empleado INT, IN P_rol INT)
BEGIN
	...
	IF(P_id IS NULL OR P_id=0) THEN
		...
		IF(@cantUsuarios > 0)THEN
			SELECT 'FALSE' AS respuesta, 'Usuario existente' AS 'error';
		ELSE
	...
	ELSE
		...
		IF(P_nombre!="" OR P_nombre IS NOT NULL)THEN
			...
			END IF;
		END IF;
		IF((P_pass!="" OR P_pass IS NOT NULL) AND NOT V_error) THEN
			IF(@sqlQuery!="")THEN
				SET @sqlQuery = CONCAT(@sqlQuery, ", ");
			END IF;
		...
		END IF;
		IF((P_rol!="" OR P_rol IS NOT NULL) AND NOT V_error)THEN
			IF(@sqlQuery!="")THEN
				SET @sqlQuery = CONCAT(@sqlQuery, ", ");
			END IF;
...
-- Dump completed on 2016-05-24 11:04:14
