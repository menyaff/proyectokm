<?php
	...
	switch($accion){
		case "select":
			...
			$sql = $BD->doSP("SPQ_cat_tiposEvento",$WS->getParametro());
			$query = $BD->query($sql);
			...
			break;
		case "update":
			$WS = new webservice("hdnId,iClave,iNombre");

			$query = $BD->doSP("SPU_cat_tiposEvento",$WS->getParametro());
		...
		case "delete":
			...
			$query = $BD->doSP("SPD_cat_tiposEvento",$parametros);
